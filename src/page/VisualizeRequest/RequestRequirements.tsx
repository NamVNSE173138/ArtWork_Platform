...
import { requestNotification } from "../../api/notificate/notifcationAPI";
interface Pin {
  _id: string;
  artworkId: number;
  userId: number;
...
export default function RequestRequirements() {
  ...
  const requestForm = useFormik({
    ...
    onSubmit: async (values: FormValues) => {
      ...
      await axios
        .post(`http://localhost:5000/userRequests`, {
          name: values.name,
          quantity: 1,
        ...
        .then((res: AxiosResponse) => {
          ...
          messageApi
            ...
            .then(() =>
              message.success(
                "Your request has been sent. Please stay tuned for responses from the artist",
                3
              )
            );
          setTimeout(() => {
          ...
          requestNotification(artist._id);
        })
        .catch((err) => console.log(err));
    },
  });
      ...
      <form onSubmit={requestForm.handleSubmit} style={{ minWidth: "100%" }}>
        <Flex justify="center" align="center" className={styles.requestForm}>
          ...
          >
            <Flex vertical justify="center" align="center">
              <Flex justify="space-evenly" align="center" gap={20}>
                ...
                <Text
                  strong
                  style={{ fontSize: "150%", minWidth: "max-content" }}
                  id={styles.artistName}
                  onClick={() => {
                    navigate(`../../artistList/${artist._id}`);
                  }}
                >
                  {artist.nickname}
                </Text>
          ...
          <Flex
            ...
            vertical
          >
            <Title style={{ fontSize: "180%", fontWeight: "700" }}>
              Tell the artist about your art...
            </Title>

            <Flex vertical style={{ width: "80%", margin: "0 auto" }} gap={10}>
              <Text>
                Name of the artwork <strong style={{ color: "red" }}>*</strong>
              </Text>
              <input
